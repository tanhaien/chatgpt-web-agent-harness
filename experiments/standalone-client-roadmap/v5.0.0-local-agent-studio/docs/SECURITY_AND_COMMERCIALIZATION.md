...
4. State-changing requests require `application/json`, preventing simple
...
7. Support bundles recursively redact credentials and omit raw tool arguments
   and results from the event list.
8. SQLite persists threads without putting API credentials in the database.

This is defense in depth, not an operating-system sandbox. The Stable desktop
app must add an OS-enforced workspace sandbox, network allowlists, typed IPC,
signed installers, and signed updates before customer release.
...
- Electron renderer sandbox with `nodeIntegration=false` and
...
4. Thao tác thay đổi chỉ nhận `application/json`, chặn form hoặc request
   `text/plain` cross-origin đơn giản.
5. App gửi CSP, anti-framing, no-sniff, no-referrer và permissions policy.
6. Remote MCP bị tắt trừ khi operator chủ động bật.
7. Support Bundle redaction đệ quy và bỏ raw tool args/results khỏi event list.
8. SQLite lưu thread nhưng không lưu API credential.

Đây là defense in depth, chưa phải sandbox cấp hệ điều hành. Trước khi phát hành
Stable cho khách, desktop app phải có OS sandbox, network allowlist, typed IPC,
installer có chữ ký và update có chữ ký.
...
Claims bắt buộc gồm `product`, `licenseId`, `customerId` và `edition`. Có thể có
thêm `issuedAt`, `notBefore`, `expiresAt` và `features`.
...
Release pipeline phải dùng `npm ci`, chạy test/audit trên ba hệ điều hành, tạo
SHA-256 manifest, ký manifest bên ngoài repo, tạo SBOM/provenance, ký artifact
theo từng nền tảng, công bố checksum và hỗ trợ rollback update.
